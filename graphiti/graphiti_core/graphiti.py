...
class Graphiti:
    def __init__(
        ...
        prompt_library : PromptLibrary | None, optional
            A custom prompt library for entity/edge extraction. If not provided,
            the default graphiti prompts will be used. Use `create_prompt_library()`
            to create a custom library with specific prompt overrides.
            ...
            try:
                ...
                if update_communities:
                    communities, community_edges = await semaphore_gather(
                        *[
                            update_community(self.driver, self.llm_client, self.embedder, node, self.prompt_library)
                            for node in nodes
                        ],
        ...
        with self.tracer.start_span('add_episode_bulk') as bulk_span:
            ...
            try:
                ...
                edges_by_episode = await dedupe_edges_bulk(
                    ...
                    episode_context,
                    [node for nodes in nodes_by_episode.values() for node in nodes],
                    edge_types or {},
                    edge_type_map or edge_type_map_default,
                )
