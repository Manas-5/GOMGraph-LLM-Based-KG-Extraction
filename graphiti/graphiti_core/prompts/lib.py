...
def create_prompt_library(
    overrides: dict[str, dict[str, PromptFunction]] | None = None
) -> PromptLibrary:
    """
    Create a prompt library with optional overrides.

    Parameters
    ----------
    overrides : dict[str, dict[str, PromptFunction]] | None
        Dictionary mapping prompt type names (e.g., 'extract_nodes') to
        dictionaries mapping version names (e.g., 'extract_text') to
        custom prompt functions.

    Returns
    -------
    PromptLibrary
        A new prompt library instance with the specified overrides applied.

    Example
    -------
    >>> def my_extract_text(context: dict) -> list[Message]:
    ...     return [Message(role='system', content='Custom prompt')]
    >>>
    >>> custom_library = create_prompt_library({
    ...     'extract_nodes': {'extract_text': my_extract_text}
    ... })
    """
    # Start with a copy of the default implementation
    library_impl: PromptLibraryImpl = {  # type: ignore[assignment]
        'extract_nodes': dict(extract_nodes_versions),
        'dedupe_nodes': dict(dedupe_nodes_versions),
        'extract_edges': dict(extract_edges_versions),
        'dedupe_edges': dict(dedupe_edges_versions),
        'invalidate_edges': dict(invalidate_edges_versions),
        'extract_edge_dates': dict(extract_edge_dates_versions),
        'summarize_nodes': dict(summarize_nodes_versions),
        'eval': dict(eval_versions),
    }

    # Apply overrides
    if overrides:
        for prompt_type, versions in overrides.items():
            if prompt_type in library_impl:
                library_impl[prompt_type].update(versions)  # type: ignore[literal-required]

    return PromptLibraryWrapper(library_impl)  # type: ignore[return-value]
