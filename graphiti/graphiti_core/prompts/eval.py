...
def eval_add_episode_results(context: dict[str, Any]) -> list[Message]:
    sys_prompt = """You are a judge that determines whether a baseline graph building result from text is better
        than a candidate graph building result based on the same text."""

    user_prompt = f"""
    Given the following PREVIOUS TEXT and CURRENT TEXT, determine if the BASELINE graph data extracted from the 
    text is higher quality than the CANDIDATE graph data extracted from the text.
