...
def edge(context: dict[str, Any]) -> list[Message]:
    return [
        Message(
            ...
            '2. Treat the CURRENT TIME as the time the CURRENT TEXT was processed. All temporal information should be extracted relative to this time.',
        ),
        Message(
            role='user',
            content=f"""
...
# TASK
Extract all factual relationships between the given ENTITIES based on the CURRENT TEXT.
Only extract facts that:
- involve two DISTINCT ENTITIES from the ENTITIES list,
- are clearly stated or unambiguously implied in the CURRENT TEXT,
    and can be represented as edges in a knowledge graph.
- Facts should include entity names rather than pronouns whenever possible.
- The FACT TYPES provide a list of the most important types of facts, make sure to extract facts of these types
- The FACT TYPES are not an exhaustive list, extract all facts from the text even if they do not fit into one
    of the FACT TYPES
- The FACT TYPES each contain their fact_type_signature which represents the source and target entity types.

You may use information from the PREVIOUS TEXT only to disambiguate references or support continuity.
...
Given the above TEXT, list of EXTRACTED ENTITIES entities, and list of EXTRACTED FACTS; 
...
def extract_attributes(context: dict[str, Any]) -> list[Message]:
    return [
        ...
        Given the above TEXT, its REFERENCE TIME, and the following FACT, update any of its attributes based on the information provided
        ...
        2. Only use the provided TEXT and FACT to set attribute values.
