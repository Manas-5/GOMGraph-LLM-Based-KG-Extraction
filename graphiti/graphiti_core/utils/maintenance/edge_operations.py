...
from graphiti_core.prompts.lib import PromptLibrary, prompt_library as default_prompt_library
...
_CLEARED_EDGE_DEDUPE_FILES: set[Path] = set()
...
async def extract_edges(
    ...
    # Fallback: if the main pass produced zero edges, retry once with no
    # previous-episode context to rule out prompt-length or context-mismatch
    # issues that occasionally surface on long runs (e.g., chunk 7+).
    if len(edges_data) == 0:
        try:
            context_no_prev = dict(context)
            context_no_prev['previous_episodes'] = []
            context_no_prev['custom_prompt'] = ''
            llm_response_retry = await llm_client.generate_response(
                prompt_library.extract_edges.edge(context_no_prev),
                response_model=ExtractedEdges,
                max_tokens=extract_edges_max_tokens,
                group_id=group_id,
                prompt_name='extract_edges.edge.retry_no_prev',
            )
            retry_edges = ExtractedEdges(**llm_response_retry).edges
            if retry_edges:
                logger.warning(
                    'Edge extraction fallback succeeded: %d edges without previous_episodes for %s',
                    len(retry_edges),
                    episode.name,
                )
                edges_data = retry_edges
        except Exception:
            logger.warning('Edge extraction fallback failed for %s', episode.name, exc_info=True)

    end = time()
    try:
        if len(edges_data) == 0:
            logger.debug(
                '[EDGES] extracted=0 episode=%s len_chars=%d',
                episode.name,
                len(episode.content or ''),
            )
        else:
            sample_facts = [e.fact for e in edges_data[:3]]
            logger.debug(
                '[EDGES] extracted=%d episode=%s sample_facts=%s duration_ms=%.1f',
                len(edges_data),
                episode.name,
                sample_facts,
                (end - start) * 1000,
        ...
        logger.debug('Extracted new edges: %s in %.1f ms', edges_data, (end - start) * 1000)
...
async def resolve_extracted_edges(
    ...
    fast_duplicates: list[tuple[EntityEdge, EntityEdge]] = []
    prompt_library = clients.prompt_library
    ...
    for edge in extracted_edges:
        ...
        if key not in seen:
            ...
            fast_duplicates.append((edge, seen[key]))
    ...
    duplicate_pairs: list[tuple[str, EntityEdge, EntityEdge]] = []
    for result in results:
        resolved_edge = result[0]
        invalidated_edge_chunk = result[1]
        duplicate_edges_chunk = result[2]

        resolved_edges.append(resolved_edge)
        invalidated_edges.extend(invalidated_edge_chunk)
        duplicate_pairs.extend([('llm_resolve', dup, resolved_edge) for dup in duplicate_edges_chunk])
    ...
    _write_edge_dedupe_csv(
        episode,
        entities,
        [('fast_exact', alias, canon) for alias, canon in fast_duplicates] + duplicate_pairs,
    )
...
async def resolve_extracted_edge(
    ...
    """
    # Use default prompt library if not provided
...
def _chunk_label(episode: EpisodicNode | None) -> str:
    if episode is None:
        return 'chunk_unknown'
    name = episode.name or ''
    if '-part_' in name:
        try:
            suffix = name.split('-part_')[-1]
            num = int(suffix)
            return f'chunk_{num}'
        except Exception:
            return f'chunk_{episode.uuid}'
    return f'chunk_{episode.uuid}'


def _edge_log_paths(group_id: str, episode: EpisodicNode | None) -> tuple[Path, Path]:
    base = Path.cwd() / 'logs' / group_id
    chunk_dir = base / 'chunks'
    chunk_dir.mkdir(parents=True, exist_ok=True)
    label = _chunk_label(episode)
    return base / 'dedupe_edges.csv', chunk_dir / f'{label}_dedupe_edges.csv'


def _write_edge_dedupe_csv(
    episode: EpisodicNode,
    entities: list[EntityNode],
    duplicate_pairs: list[tuple[str, EntityEdge, EntityEdge]],
) -> None:
    ...
    aggregate_csv, chunk_csv = _edge_log_paths(group_id, episode)
    agg_new = aggregate_csv not in _CLEARED_EDGE_DEDUPE_FILES
    chunk_new = chunk_csv not in _CLEARED_EDGE_DEDUPE_FILES

    def edge_row(stage: str, canonical: EntityEdge, alias: EntityEdge) -> list[str]:
        now = datetime.now(timezone.utc).isoformat()
        canon_src = node_map.get(canonical.source_node_uuid)
        canon_tgt = node_map.get(canonical.target_node_uuid)
        alias_src = node_map.get(alias.source_node_uuid)
        alias_tgt = node_map.get(alias.target_node_uuid)
        excerpt = (episode.content or '')[:200].replace('\n', ' ')
        return [
            now,
            stage,
            canonical.uuid,
            canonical.fact,
            canonical.source_node_uuid,
            canon_src.name if canon_src else '',
            canonical.target_node_uuid,
            canon_tgt.name if canon_tgt else '',
            alias.uuid,
            alias.fact,
            alias.source_node_uuid,
            alias_src.name if alias_src else '',
            alias.target_node_uuid,
            alias_tgt.name if alias_tgt else '',
            episode.uuid,
            excerpt,
        ]

    def write_rows(path: Path, is_new: bool, rows: list[list[str]]):
        mode = 'w' if is_new else 'a'
        if is_new:
            _CLEARED_EDGE_DEDUPE_FILES.add(path)
        with path.open(mode, newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if is_new:
                writer.writerow(EDGE_DEDUPE_HEADERS)
            for row in rows:
                writer.writerow(row)

    rows: list[list[str]] = []
    for stage, alias, canon in duplicate_pairs:
        rows.append(edge_row(stage, canon, alias))

    try:
        write_rows(aggregate_csv, agg_new, rows)
        write_rows(chunk_csv, chunk_new, rows)
    except Exception as exc:
        logger.debug('Failed to write edge dedupe CSV: %s', exc)
