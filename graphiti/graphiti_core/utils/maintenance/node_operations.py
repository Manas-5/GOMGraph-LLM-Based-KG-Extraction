...
# Track which files we have already truncated in this process so reruns start clean
_CLEARED_NODE_DEDUPE_FILES: set[Path] = set()
...
async def _resolve_with_llm(
    ...
    for resolution in node_resolutions:
        ...
        state.merge_stage[extracted_node.uuid] = 'llm'
...
async def resolve_extracted_nodes(
    ...
    if log_dedupe:
        _write_node_dedupe_csv_single(state.uuid_map, state.resolved_nodes, episode, state.merge_stage)
...
async def extract_attributes_from_node(
    ...
    llm_response = await _extract_entity_attributes(
        llm_client, node, prompt_library, episode, previous_episodes, entity_type
    )

    # Extract summary if needed
    await _extract_entity_summary(
        llm_client, node, prompt_library, episode, previous_episodes, should_summarize_node
    )
...
def _chunk_label(episode: EpisodicNode | None) -> str:
    """Derive a stable chunk label from episode.name; fallback to uuid."""
    if episode is None:
...
def _write_node_dedupe_csv_single(
    uuid_map: dict[str, str],
    resolved_nodes: list[EntityNode | None],
    episode: EpisodicNode | None,
    merge_stage: dict[str, str] | None = None,
) -> None:
    """Append per-call dedupe results to both aggregate and per-episode CSVs (detailed)."""
    if not uuid_map:
        return

    group_id = episode.group_id if episode is not None else None
    if not group_id:
        return

    episode_uuid = episode.uuid if episode is not None else ''
    episode_excerpt = (episode.content or '')[:200].replace('\n', ' ') if episode else ''

    log_dir = Path.cwd() / 'logs' / group_id
    try:
        log_dir.mkdir(parents=True, exist_ok=True)
    except Exception:
        logger.debug('Unable to create dedupe log directory at %s', log_dir)
        return

    aggregate_csv = log_dir / 'dedupe_nodes.csv'
    per_chunk_dir = log_dir / 'chunks'
    per_chunk_dir.mkdir(parents=True, exist_ok=True)

    chunk_label = _chunk_label(episode)
    chunk_csv = per_chunk_dir / f'{chunk_label}_dedupe_nodes.csv'

    aggregate_new = aggregate_csv not in _CLEARED_NODE_DEDUPE_FILES
    chunk_new = chunk_csv not in _CLEARED_NODE_DEDUPE_FILES

    alias_metadata: dict[str, tuple[str, str]] = {}
    canonical_lookup: dict[str, EntityNode] = {}
    for node in resolved_nodes:
        if node is None:
            continue
        alias_metadata[node.uuid] = (node.name, episode_uuid)
        canonical_lookup.setdefault(node.uuid, node)

    grouped: dict[str, list[str]] = {}
    for alias_uuid, canonical_uuid in uuid_map.items():
        grouped.setdefault(canonical_uuid, []).append(alias_uuid)
    ...
    def _write_rows(path: Path, is_new: bool):
        mode = 'w' if is_new else 'a'
        if is_new:
            _CLEARED_NODE_DEDUPE_FILES.add(path)
        with path.open(mode, newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            if is_new:
                writer.writerow(headers)
            for canonical_uuid, aliases in grouped.items():
                if len(aliases) <= 1:
                    continue
                canonical_name_obj = canonical_lookup.get(canonical_uuid, None)
                canonical_name = canonical_name_obj.name if canonical_name_obj else ''
                canonical_summary = canonical_name_obj.summary if canonical_name_obj else ''
                for alias_uuid in aliases:
                    now = datetime.now(timezone.utc).isoformat()
                    alias_name, alias_episode = alias_metadata.get(alias_uuid, ('', ''))
                    alias_summary = ''
                    alias_node = canonical_lookup.get(alias_uuid)
                    if alias_node:
                        alias_summary = alias_node.summary
                    stage_val = (merge_stage or {}).get(alias_uuid, '')
                    writer.writerow(
                        [
                            now,
                            stage_val,
                            ...
                            alias_summary,
                            alias_episode,
                            episode_excerpt if alias_episode else '',
                        ]
                    )

    try:
        _write_rows(aggregate_csv, aggregate_new)
        _write_rows(chunk_csv, chunk_new)
    except Exception as exc:
        logger.debug('Failed to write dedupe CSV: %s', exc)
...
#     if log_dedupe:
#         _write_node_dedupe_csv_single(state.uuid_map, state.resolved_nodes, episode)
