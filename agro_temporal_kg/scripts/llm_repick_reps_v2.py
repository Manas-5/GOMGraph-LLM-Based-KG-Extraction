# python3 llm_repick_reps_v2.py \
#     --cluster-dir "/mnt/diskGum/manas/graph-dedup-patch/agro_temporal_kg/scripts/cluster_runs" \
#     --out-dir "/mnt/diskGum/manas/graph-dedup-patch/agro_temporal_kg/scripts/llm_reps_v2" \
#     --distance-mode \
#     --model "llama3.1:70b" \
...
- Also emits per-threshold preprocessing artifacts: normalized label groups and near-duplicate similarity candidates.
...
def normalize_name(s: str) -> str:
    """Normalize a relation label for embedding/similarity use."""
    return normalize_relation(s)
...
def build_preprocessing_artifacts(map_df: pd.DataFrame, out_dir: Path, prefix: str, similarity_threshold: float):
    """
    Create normalized groupings and near-duplicate candidates from a mapping dataframe.
    Writes two CSVs:
    - {prefix}_exact_normalized_groups.csv: collapsed counts by normalized label with examples.
    - {prefix}_similarity_candidates.csv: pairs of normalized labels with cosine similarity >= threshold.
    """
    required = {"name", "count"}
...
def main():
    ...
    ap.add_argument("--sim-threshold", type=float, default=DEFAULT_SIMILARITY_THRESHOLD,
                    help="Cosine similarity cutoff for near-duplicate detection when preprocessing mapping files")
    ...
    ap.add_argument("--out-dir", default="llm_reps_v2", help="Output directory (default set for v2 run)")
    ...
    # Precompute embeddings once for all unique member names (if requested)
    if args.save_embeddings:
        emb_path = out_dir / "all_names_embeddings.npz"
        normalized_list_path = out_dir / "normalized_names_list.csv"
        
        # Always recompute embeddings from normalized names (delete old ones if present)
        if emb_path.exists():
            print(f"Deleting old embeddings at {emb_path} to recompute with normalized names...")
            emb_path.unlink()
        
        # Extract unique member names from mapping file (use the first/any threshold since all members are the same)
        print("Extracting unique member names from mapping file...")
        first_sim = 1.0 - thresholds[0] if (args.distance_mode or args.thresholds is None) else thresholds[0]
        mapping_file = find_mapping_file(cluster_dir, first_sim)
        map_df = pd.read_csv(mapping_file)
        raw_names_list = sorted(map_df["name"].dropna().unique().tolist())
        print(f"Found {len(raw_names_list)} unique member names")
        
        # Normalize first, then embed the normalized texts
        norm_texts = [normalize_name(n) for n in raw_names_list]
        print(f"Embedding {len(raw_names_list)} unique names once with {args.embed_model} (after normalization) ...")
        embs = embed_texts(args.embed_model, norm_texts, args.base_url, args.api_key or os.environ.get("OPENROUTER_API_KEY", ""))
        embs = np.asarray(embs, dtype=np.float32)
        
        # Store both raw names and their normalized versions
        np.savez(emb_path, names=np.array(raw_names_list), normalized_names=np.array(norm_texts), embeddings=embs)
        print(f"Saved embeddings to {emb_path}")
        
        # Also save normalized names list as CSV for reference
        norm_df = pd.DataFrame({
            "raw_name": raw_names_list,
            "normalized_name": norm_texts
        })
        norm_df.to_csv(normalized_list_path, index=False)
        print(f"Saved normalized names list to {normalized_list_path}")

    # Verification step: ensure all mapping files have consistent entity coverage
    print("\n" + "="*80)
    print("VERIFICATION: Checking entity consistency across all thresholds...")
    print("="*80)
    
    reference_names = None
    mapping_names_per_threshold = {}
    ...
    for thr in thresholds:
        ...
        if mapping_file is None:
            print(f"[warn] Missing mapping file for sim={sim:.2f}; skipping verification for this threshold.")
            continue
        
        map_df = pd.read_csv(mapping_file)
        threshold_names = set(map_df["name"].dropna().unique().tolist())
        mapping_names_per_threshold[thr] = threshold_names
        
        if reference_names is None:
            reference_names = threshold_names
            print(f"[ref] Threshold {thr:.2f}: {len(reference_names)} unique names (reference)")
        else:
            if threshold_names != reference_names:
                missing_in_current = reference_names - threshold_names
                extra_in_current = threshold_names - reference_names
                if missing_in_current:
                    print(f"[err] Threshold {thr:.2f}: MISSING {len(missing_in_current)} names: {sorted(list(missing_in_current)[:5])}...")
                if extra_in_current:
                    print(f"[err] Threshold {thr:.2f}: EXTRA {len(extra_in_current)} names: {sorted(list(extra_in_current)[:5])}...")
            else:
                print(f"[ok]  Threshold {thr:.2f}: {len(threshold_names)} unique names (consistent)")
    
    # Check embeddings consistency
    if args.save_embeddings:
        embedded_names = set(raw_names_list)
        if embedded_names != reference_names:
            missing_in_embeddings = reference_names - embedded_names
            extra_in_embeddings = embedded_names - reference_names
            if missing_in_embeddings:
                print(f"[err] Embeddings MISSING {len(missing_in_embeddings)} names: {sorted(list(missing_in_embeddings)[:5])}...")
            if extra_in_embeddings:
                print(f"[err] Embeddings EXTRA {len(extra_in_embeddings)} names: {sorted(list(extra_in_embeddings)[:5])}...")
        else:
            print(f"[ok]  Embeddings: {len(embedded_names)} names (consistent with mapping files)")
    ...
    for thr in tqdm(thresholds, desc="Processing thresholds", unit="thr"):
        ...
        # Preprocessing: normalized grouping + similarity candidates for this mapping file
        map_df = pd.read_csv(mapping_file)
        pre_prefix = f"thr_{thr:.2f}"
        exact_path, sim_path = build_preprocessing_artifacts(map_df, out_dir, pre_prefix, args.sim_threshold)
        print(f"  Preprocessing artifacts -> {exact_path}, {sim_path}")
